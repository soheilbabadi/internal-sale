...
@Subselect("""
		...
		FROM TBL_IME_TRADE tit
		    ...
		    WHERE x.rn = 1
		      AND (x.C_WORKFLOW_APPROVE_STATUS IS NULL OR x.C_WORKFLOW_APPROVE_STATUS NOT IN ('IN_PROGRESS', 'ACCEPTED'))
		...
		WHERE tit.CONTRACT_DATE > '1405/01/01'
		  ...
		  AND NOT EXISTS (
		    ...
		    WHERE latest.C_PAYMENT_CODE = tit.PAYMENT_CODE
		      AND latest.rn = 1
		      AND latest.C_WORKFLOW_APPROVE_STATUS IN ('IN_PROGRESS', 'ACCEPTED')
		  )
		ORDER BY tit.CONTRACT_DATE DESC
		""")
public class TradeExtractModel implements Serializable {
