...
public class CrmController {
	...
	public ResponseEntity<byte[]> exportRemittancePdf(
			...
			@PathVariable long id) {
		try {
			byte[] docxBytes = historyService.exportRemittanceDoc(id);
			XWPFDocument document = new XWPFDocument(new ByteArrayInputStream(docxBytes));
			var pdf = convertDocListToPdf(Collections.singletonList(document));
			return ResponseEntity.ok()
					.header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"remittance_" + id + ".pdf\"")
					.contentType(MediaType.APPLICATION_PDF)
					.contentLength(Objects.requireNonNull(pdf.getBody()).length)
					.body(pdf.getBody());
	...
	public ResponseEntity<byte[]> exportProformaPdf(
			...
			@PathVariable long id) {
		try {
			byte[] docxBytes = historyService.exportProformaDoc(id);
			XWPFDocument document = new XWPFDocument(new ByteArrayInputStream(docxBytes));
			var pdf = convertDocListToPdf(Collections.singletonList(document));
			return ResponseEntity.ok()
